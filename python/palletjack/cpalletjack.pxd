...
cdef extern from "palletjack.h":
    ...
    cdef shared_ptr[CFileMetaData] ReadSchema(const char *index_file_path, const vector[uint32_t] column_indices, const vector[string] column_names) except + nogil
    cdef shared_ptr[CFileMetaData] ReadSchema(const unsigned char *index_data, size_t index_data_length, const vector[uint32_t] column_indices, const vector[string] column_names) except + nogil
